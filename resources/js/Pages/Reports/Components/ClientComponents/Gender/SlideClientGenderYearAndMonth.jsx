...
import ReportClientGenderYearAndMonthList from "@/Pages/Reports/Components/ClientComponents/Gender/ReportClientGenderYearAndMonthList.jsx";


export default function SlideClientGenderYearAndMonth({gender, year,month}) {
    const [open, setOpen] = useState(false)
    const {auth} = usePage().props
    const { errors } = usePage().props
    const monthNames = ["Meses","January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
                                            ...
                                            <div className="relative mt-6 flex-1 px-4 sm:px-6">
                                                <div className="max-w-2xl mx-auto px-4 lg:max-w-none lg:px-0">
                                                    <div className="mt-6 grid grid-cols-1 gap-4 justify-end">
                                                        <ReportClientGenderYearAndMonthList gender={gender} center={auth.user.centro_id} year={year} month={month} />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

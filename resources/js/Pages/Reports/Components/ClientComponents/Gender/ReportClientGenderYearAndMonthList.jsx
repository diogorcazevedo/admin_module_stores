...
export default function ReportClientGenderYearAndMonthList({gender, center,year,month}) {
    ...
    useEffect(() => {
        ...
        const url =  `https://administracaodosistema.com.br/reports/api/report/client/month_gender/`+gender+"/"+year+'/'+month+"/"+center

        const getData = async () => {
            try {
                const response = await axios.get(url);
                const sorted = orderBy(response.data.data, ["total"], 'desc');
                setData(sorted);
        ...
        <>
            ...
            {!loading &&
                <div className="grid grid-cols-1 gap-4">
                    <div>
                        <p className="bg-teal-800 py-2 px-2 text-white">VENDAS POR QTD: {month_name} / {year} </p>
                    </div>
                    <div>

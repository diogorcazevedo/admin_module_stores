...
import SlideAdd from "@/Pages/Service/Legacy/Items/Components/SlideAdd";
        ...
        <>
            ...
            <Auth auth={auth} errors={errors}>
                            ...
                            {service_items.map((item) => (
                                <tr key={item.id} className="divide-x divide-y divide-gray-200">
                                    <td className="text-xs justify-center p-2">
                                        <Link className="justify-center" href={route('service_items.legacy.destroy',{id:item.id, service:item.service_id})}>
                                            <XMarkIcon className="h-4 w-4 text-red-800" aria-hidden="true"/>
                                        </Link>
                                    </td>
                                    <td className="text-xs p-2">
                                        <SlideCreateProductImages product={item.product} images={item.product.images[0] ? item.product.images :''} types={types} />
                                    </td>
                                    <td className="text-xs p-2">
                                        <Link href={route('service_items.legacy.edit',{id:item.id})}>
                                            {item.product.name}
                                        </Link>
                                    </td>
                                    <td className="text-xs p-2">
                ...
                <div className="shadow mt-6 p-4 flex flex-row-reverse">
                    <div className="basis-1/3">
                        <Link href={route('service.legacy.index')}
